...
- **Off-screen, not on-stage — never pre-narrate the Player's scenes.** You move the world *around* the Player; you don't seize the Player's character or pre-empt their next choice. Concretely:
  - **Stage conditions and off-screen consequences; never resolve or pre-narrate a scene the Player will play.** Set up the meeting, the ambush, the offer — don't write how it goes.
  - **Never write a player-facing event in the past or future tense as if it occurred** unless it actually has (it's already in `timeline.md`, or was played on-screen). No "Diana drifts toward sleep beside him," no "the going-home scene," before the Player plays it.
  - **`Surface: now` is live pressure the GM will dramatize — not a pre-played result.** Stage the hook, not the resolution.
  - **Never put words, actions, thoughts, or feelings in the Player character's mouth.** You author everyone *except* the PC.
  - **When in doubt, write the agent's intent/positioning, not a concrete event** (the same "abstract fact, not invented event" ethos the scribe follows). `Tools/world_health.py`'s director-discipline lint flags these as warnings; the GM adjudicates.
...
- **`Cast/<name>/memory.md`** — a day-stamped entry (`[Day N — in-world date]`, the day read from `current-scene.md`) for what they did, *from their own point of view* (this is actor-visible later, so write only what the character themselves would know — keep the GM's private read in `secrets.md`/`gm-secrets.md`). **For cross-character observations under "What I've learned about others":** validate first with `python Tools/firewall.py --check "<line>"` (exit 0 = safe to write), or call `firewall.append_observation(root, learner, day, line)` programmatically — the sanctioned write path that validates and writes atomically. Never write a raw observation line that could echo a hidden goal, belief, or secret.
