...
- **`sheet.md`** — *optional, GM-only.* Mechanical stats (dice pools, Health levels, signature supernatural powers) for an NPC who'll face contested rolls or a fight, so the opposition is consistent and fair. Like `secrets.md`, it's never handed to the actor. Skip it for anyone who'll never be rolled against.
- **`drives.md`** — *optional, GM-only.* Off-screen goals and a small state machine that make this character **living** — pursued automatically by the world tick (`Tools/world_tick.py`) and the `world-director` while the Player isn't watching. Like `secrets.md` and `sheet.md`, never handed to the actor. Add it only for characters the story leans on; see `CLAUDE.md` → "The living world."
