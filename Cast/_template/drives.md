...
## Reflection notes
Higher-level **beliefs** this agent has drawn over past ticks — about the Player, rivals, or their own situation. (The Generative-Agents "reflection" step: synthesizing events into beliefs keeps a long-running character coherent instead of drifting.) Start empty; the loop appends here automatically — a **Claude director** (`world-director-lite` on Sonnet, or `world-director` for a pivot) synthesises a belief whenever the agent completes a phase (an FSM transition) or fills a clock, and may then *re-plan* from it (retarget the goal, flip a relationship). See `CLAUDE.md` → "The agent loop."
