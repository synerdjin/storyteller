...
|---|---|---|
| **Engine** — shipped by the template, never edited by play | **Overwritten** with the latest version | `CLAUDE.md`, `UPDATING.md`, `VERSION`, `CHANGELOG.md`, `LICENSE`, `README.md`, `.gitignore`, `.claude/agents/` (all agents), `Tools/dice.py`, `Tools/world_tick.py`, `Tools/world_scribe.py`, `Tools/story_compile.py`, `Tools/cultural_profile.py`, `Tools/ledger.py`, `Tools/social.py`, `Tools/world_health.py`, `Tools/world_dashboard.py`, `Tools/memory_index.py`, `Tools/memory_search.py`, `Tools/local_client.py`, `Tools/local_config.py`, `Tools/local-agents/`, `Cast/README.md`, `Cast/CRAFTING-NPCS.md`, `Cast/_template/`, `Story/chapters/_TEMPLATE.md`, `Sourcebooks/README.md`, `Sourcebooks/_digests/.gitkeep`, `Sourcebooks/_digests/_TEMPLATE.md`, `Character/README.md`, `Character/portraits/.gitkeep` |
| **Your story (scaffolds)** — shipped empty, filled by play | **Never overwritten.** If a new version changes the *structure*, the GM splices it in interactively | `PLAYER-NOTES.md`, `Game/system.md`, `Game/boundaries.md`, `Game/campaign.md`, `Game/world.md`, `Game/timeline.md`, `Game/current-scene.md`, `Game/threads.md`, `Game/plots.md`, `Game/gm-secrets.md`, `Game/world-state.md`, `Game/developments.md`, `Game/ledgers.md`, `Story/index.md` |
| **The Ant Farm app** — engine code, shipped and versioned with the engine | **Overwritten** (source only). Its runtime store and build artifacts are gitignored, so they're never in the repo and an update can't touch them | `app/` (the FastAPI backend + React front-end). Untracked/gitignored and therefore safe: `app/antfarm.db`, `app/web/node_modules/`, `app/web/dist/` |
...
```
git checkout engine/main -- CLAUDE.md UPDATING.md VERSION CHANGELOG.md LICENSE README.md .gitignore .claude/agents Tools/dice.py Tools/world_tick.py Tools/world_scribe.py Tools/world_health.py Tools/story_compile.py Tools/cultural_profile.py Tools/ledger.py Tools/social.py Tools/world_dashboard.py Tools/memory_index.py Tools/memory_search.py Tools/local_client.py Tools/local_config.py Tools/local-agents Cast/README.md Cast/CRAFTING-NPCS.md Cast/_template Story/chapters/_TEMPLATE.md Sourcebooks/README.md Sourcebooks/_digests/.gitkeep Sourcebooks/_digests/_TEMPLATE.md Character/README.md Character/portraits/.gitkeep app
```
This overwrites only the named paths. Your `Game/*.md`, `Cast/<name>/`, `Character/sheet.md`, `.claude/settings.json`, and `Sourcebooks/` content are not named, so they don't move. (If the CHANGELOG says a file was **removed** in this release, delete your local copy by hand.) The trailing `app` restores the Ant Farm app's *source* — its gitignored runtime store (`app/antfarm.db`) and build artifacts aren't tracked, so `git checkout` leaves any worlds you've created alone.
